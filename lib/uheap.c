...
// Tracking arrays for user heap allocations
static bool uarr[(USER_HEAP_MAX - USER_HEAP_START) / PAGE_SIZE] = {0};
static int usizeAfter[(USER_HEAP_MAX - USER_HEAP_START) / PAGE_SIZE] = {0};
...
void* malloc(uint32 size)
{
    ...
    uint32 needed = (size + PAGE_SIZE - 1) / PAGE_SIZE;
    int bestStart = -1;
    int bestCount = ((USER_HEAP_MAX - USER_HEAP_START) / PAGE_SIZE) + 1; // Initialize to max possible size
    int tmpStart = 0;
    int count = 0;
    ...
    for (int i = bestStart; i < bestStart + needed; i++)
    {
        uarr[i] = 1;
        usizeAfter[i] = needed - (i - bestStart) - 1;
    }

    uint32 va = USER_HEAP_START + bestStart * PAGE_SIZE;
...
void free(void* virtual_address)
{
    ...
    uint32 num_pages = usizeAfter[index] + 1;

    for (int i = index; i < index + num_pages; i++)
    {
        if (i >= (USER_HEAP_MAX - USER_HEAP_START) / PAGE_SIZE)
        	return;
        if (!uarr[i])
            return;
    }
