...
void __env_free_with_buffering(struct Env *e)
{
    ...
    LIST_FOREACH(ptr_fi, &modified_frame_list)
    {
        ...
        {
            // If the buffered page is modified
            if (pt_get_page_permissions(e, ptr_fi->va) & PERM_MODIFIED)
            {
                // Remove from modified_frame_list
            	bufferlist_remove_page(&modified_frame_list, ptr_fi);
            	ptr_fi->isBuffered = 0;
                ptr_fi->environment = NULL;
                ptr_fi->va = 0;
                free_frame(ptr_fi);
                pt_set_page_permissions(e, ptr_fi->va, 0, PERM_BUFFERED | PERM_MODIFIED);
            }
            else
            {
                // Non-modified buffered page
                // Remove from modified_frame_list (if mistakenly there)
            	bufferlist_remove_page(&modified_frame_list, ptr_fi);
            	ptr_fi->isBuffered = 0;
            	ptr_fi->environment = NULL;
            	ptr_fi->va = 0;
            	free_frame(ptr_fi);
                pt_set_page_permissions(e, ptr_fi->va, 0, PERM_BUFFERED);
            }
        }
    }

    // Check free_frame_list for buffered pages belonging to this environment
    LIST_FOREACH(ptr_fi, &free_frame_list)
    {
        if (ptr_fi->environment == e && ptr_fi->isBuffered)
        {
            // Flag as free frame
            ptr_fi->isBuffered = 0;
            ptr_fi->environment = NULL;
            ptr_fi->va = 0;
            // Clear BUFFERED bit in page table
            pt_set_page_permissions(e, ptr_fi->va, 0, PERM_BUFFERED);
            // Already in free_frame_list, no need to add
        }
    }
    ...
    for (uint32 va = 0; va < USTACKTOP; va += PAGE_SIZE * 1024) // Iterate over page table boundaries
    {
    ...
    struct Frame_Info* dir_frame = to_frame_info(rcr3());
    if (dir_frame != NULL)
