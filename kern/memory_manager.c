...
void allocateMem(struct Env* e, uint32 virtual_address, uint32 size)
  {
	 //cprintf("entering allocateMem\n");
	 //cprintf("size in mem: %x\n", size);
 	...
 	for (int i = 0; i < num_pages; i++)
 	 {
 		 int local= pf_add_empty_env_page( e,virtual_address,0);
 		 if(local==0)
 		 {
 			 virtual_address+=(uint32)PAGE_SIZE;
 			 found++;
 			 continue;
 		 }
 		 else
 	 ...
 	 if(found==ROUNDUP(size,PAGE_SIZE))
 		 return;
}
...
/**
 * Checks if a page table covering 'virtual_address' is empty.
 * i.e. All 1024 entries have the PRESENT bit cleared.
 *
 * @param curenv - Pointer to the environment whose page directory we're checking
 * @param virtual_address - Any virtual address covered by the desired page table
 * @return
 *      1 if the page table is empty
 *      0 if at least one entry is present
 *     -1 if the page table doesn't exist
 */
int is_page_table_empty(struct Env* curenv, uint32 virtual_address)
{
    uint32* pt;
    int result = get_page_table(curenv->env_page_directory, (void*)virtual_address, &pt);

    // Page table does not exist
    if (result != TABLE_IN_MEMORY)
        return -1;

    // Loop through all 1024 PTEs
    for (int i = 0; i < 1024; ++i)
    {
        if ((pt[i] & PERM_PRESENT) != 0)
        {
    ...
    // No present pages found  page table is empty
    return 1;
}
...
void __freeMem_with_buffering(struct Env* e, uint32 virtual_address, uint32 size)
{
    ...
    uint32 end_va = ROUNDDOWN(virtual_address + size - 1, PAGE_SIZE) + PAGE_SIZE;


    if (va < USER_HEAP_START || end_va > USER_HEAP_MAX || end_va < va)
        return;

    // unmap pages
    for (uint32 curr_va = va; curr_va < end_va; curr_va += PAGE_SIZE)
    {
        //cprintf("Processing page at va=0x%x\n", curr_va);
        uint32 permissions = pt_get_page_permissions(e, curr_va);
        uint32* page_table_ptr = NULL;
        struct Frame_Info* ptr_frame_info =
            get_frame_info(e->env_page_directory, (void*)curr_va, &page_table_ptr);


        if (permissions != 0 || ptr_frame_info != NULL)
        {
            //cprintf("Unmapping page at va=0x%x, permissions=0x%x\n", curr_va, permissions);


            if ((permissions & PERM_BUFFERED) && ptr_frame_info != NULL)
            {
    ...
    for (int i = 0; i < e->page_WS_max_size; i++)
    {
        ...
        if (!env_page_ws_is_entry_empty(e, i))
        {
            ...
            {
                //cprintf("Deleting WS entry %d at va=0x%x\n", i, ws_va);
                // unmap the frame
                unmap_frame(e->env_page_directory, (void*)ws_va);
                env_page_ws_clear_entry(e, i);
            }
        }
    }

    for (uint32 curr_va = va; curr_va < end_va; curr_va += PAGE_SIZE * 1024)
    {
        //cprintf("Checking page table for va=0x%x\n", curr_va);
        uint32 *ptr_page_table = NULL;
        get_page_table(e->env_page_directory, (void*)curr_va, &ptr_page_table);

        if (ptr_page_table != NULL)
        {
            int is_empty = is_page_table_empty(e, curr_va);
            if (is_empty) {
                uint32 pd_index = PDX(curr_va);
                uint32 page_table_phys_addr = e->env_page_directory[pd_index] & ~0xFFF;
                void *page_table_va = (void *)ptr_page_table;

                pd_clear_page_dir_entry(e, curr_va);
                pd_set_table_unused(e, curr_va);

                unmap_frame(e->env_page_directory, page_table_va);

                //cprintf("Calling kfree for page table at va=0x%x\n", page_table_va);
                kfree(page_table_va);
            }
        }
    }
