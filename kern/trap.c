...
int modified_clock(struct Env* e) {
    uint32 tmp = e->page_last_WS_index;
    bool try1 = 1;
    while (1)
    {
        if (env_page_ws_is_entry_empty(e, tmp)) {
            tmp = (tmp + 1) % e->page_WS_max_size;
            if (tmp == e->page_last_WS_index) {
                try1 = !try1;
            }
            continue;
        }

        uint32 va = e->ptr_pageWorkingSet[tmp].virtual_address;

        uint32 page_permissions = pt_get_page_permissions(e, va);

        if (try1) {
            if ((page_permissions & PERM_USED)==0 && (page_permissions & PERM_MODIFIED)==0) {
                e->page_last_WS_index = (tmp + 1) % e->page_WS_max_size;
                 return tmp;
            }
        } else {
            ...
            pt_set_page_permissions(e, va, 0, PERM_USED);
...
// Returns index in working set if found, -1 otherwise
void placement(struct Env* curenv, uint32 fault_va)
{
	uint32 page_permissions = pt_get_page_permissions(curenv, fault_va);
				...
				{
					struct Frame_Info* ptr_frame=get_frame_info(curenv->env_page_directory,(void*)fault_va,&pt);
				...
				else
				{
					...
					int r1=allocate_frame(&ptr_frame);
					if(r1!=0)
						return;
					int r2=map_frame(curenv->env_page_directory,ptr_frame,(void*)fault_va,PERM_PRESENT|PERM_WRITEABLE|PERM_USER);
					if(r2!=0)
						return;
					//read the faulted page from page file to memory.
					int r3 = pf_read_env_page(curenv,(void*) fault_va);
					//If the page does not exist
					if (r3 == E_PAGE_NOT_EXIST_IN_PF)
					{
						// then CHECK if it is a stack page.
						if(fault_va>= USTACKBOTTOM &&fault_va< USTACKTOP)
						{
							// add a new empty page with this faulted address to page file
							int r4 = pf_add_empty_env_page(curenv, ROUNDDOWN(fault_va,PAGE_SIZE), 0);
							if(r4== E_NO_PAGE_FILE_SPACE)
								return;
						}
						else panic("invalid virtual address");
					}
				}
...
void __page_fault_handler_with_buffering(struct Env *curenv, uint32 fault_va)
{
    ...
    if (env_page_ws_get_size(curenv) < curenv->page_WS_max_size)
    {
    	...
    	perm = pt_get_page_permissions(curenv, fault_va);
        if (ptr_frame_info != NULL && ptr_frame_info->isBuffered&&(perm&PERM_BUFFERED))
        {
            pt_set_page_permissions(curenv, fault_va, PERM_PRESENT, PERM_BUFFERED);
        ...
        else
        {
            ...
            r = map_frame(curenv->env_page_directory, ptr_frame_info, (void*)fault_va,
                               PERM_PRESENT | PERM_WRITEABLE | PERM_USER);
            if (r != 0)
            	return;

            // Read page from page file
            int read_r = pf_read_env_page(curenv, (void*)fault_va);
            if (read_r == E_PAGE_NOT_EXIST_IN_PF)
            {
                // Check if this is a new stack page
                if (fault_va >= USTACKBOTTOM && fault_va < USTACKTOP)
                {
                    int add_r = pf_add_empty_env_page(curenv, ROUNDDOWN(fault_va, PAGE_SIZE), 0);
                    if (add_r != 0)
                    	return;
                }
                else
                    panic("Invalid virtual address accessed");
            }
        }
    ...
    if (LIST_SIZE(&modified_frame_list) >= getModifiedBufferLength())
    {
        ...
        {
            int update_r = pf_update_env_page(fi->environment, (void*)(ROUNDDOWN(fi->va, PAGE_SIZE)), fi);
            if (update_r != 0)
            	return;

            pt_set_page_permissions(fi->environment, fi->va, PERM_BUFFERED, PERM_MODIFIED);
